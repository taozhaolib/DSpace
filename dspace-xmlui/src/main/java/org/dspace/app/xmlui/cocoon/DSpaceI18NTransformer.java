...
/**
 * This is a simple extension to the stardand Cocoon I18N transformer that
 * specializes the configuration based upon currently installed aspects.
 *
 * This transformer modified the base configuration by adding two
 * <location/> parameters for each aspect into the default catalogue. The first
 * location parameter is contained within the catalogue's base location + the
 * aspect path. The second location parameter is located inside the aspect's
 * path + "/i18n/"
 *
 * This allows aspect developers to place their default messages files inside
 * the aspect, and place translations into various languages inside the base
 * i18n/ directory.
 *
 * EXAMPLE:
 *
 * For instance let's say that the i18n transformer's configuration were as
 * follows:
...
public class DSpaceI18NTransformer extends I18nTransformer {
    ...
    /**
     * Intercept the configuration parameters comming from the cocoon sitemap
     * before they are read by the cocoon i18n transformer. We want to add in
